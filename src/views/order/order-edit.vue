					...
					<div class="q-imgs">
						...
						<div class="container q-table">
							<i-form :model="modelForm" :label-width="100">
								...
								<Form-item label="材料缺少说明" v-if="modelForm.flowState===1 && modelForm.isLack===false">
									<i-input :value.sync="modelForm.lackDesc" placeholder="请输入缺少的材料..."></i-input>
								</Form-item>
								<Form-item label="总部备注">
									<i-input :value.sync="modelForm.remark" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入备注..."></i-input>
								</Form-item>
							</i-form>
						</div>
					</div>
					<div class="q-btns">
						<i-button type="primary" :loading="lackLoading" @click="handleLack()" v-if="modelForm.flowState===1 && modelForm.isLack===false" size="large">退回补充材料</i-button>
						<i-button type="primary" :loading="saveLoading" @click="ownerSaveAppoint(true)" size="large">保存</i-button>
						<i-button type="primary" :loading="submitLoading" @click="ownerSaveAppoint(false)" size="large">提交</i-button>
      ...
      getCouponToal(){
		...
		methods:{
          ...
          if(res.success){
			...
			handleLack(){
				this.ownerSaveAppoint(true,this.lackFun);
			},
			lackFun(){
				let self=this;
				if(!self.modelForm.lackDesc){
					self.$Notice.error({
						title:'错误',
						desc:'请输入缺少的材料说明'
					});
				...
				self.lackLoading=true;
				server.lack(self.modelForm).then(res=>{
					self.lackLoading=false;
					if(res.success){
						self.$Notice.success({
							title:'退回补充材料操作成功',
							desc:res.message
